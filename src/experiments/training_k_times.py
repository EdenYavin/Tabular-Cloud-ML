        ...
        for dataset_name in config.dataset_config.names:
            ...
            for model_name in config.iim_config.name:
                ...
                if path.exists():
                    ...
                    use_kfold_cv = config.experiment_config.k_folds > 1

                    if not use_kfold_cv:
                        # Load once if single-split
                        X_train, y_train, X_test, y_test = self._collect_datasets(dataset_name=dataset_name)

                    for k_idx in tqdm(range(config.experiment_config.k_folds), total=config.experiment_config.k_folds, desc="K Trainings"):

                        if use_kfold_cv:
                            # Load specific fold
                            X_train, y_train, X_test, y_test = self._collect_datasets(dataset_name=dataset_name, fold_idx=k_idx)
                            # Fold-specific paths for history/plot
                            history_path = path / f"fold_{k_idx}" / "history.pkl"
                            plot_path = path / f"fold_{k_idx}" / f"{model_name}_{config.experiment_config.to_run}_train_plot.png"
                        else:
                            history_path = path / "history.pkl"
                            plot_path = path / f"{model_name}_{config.experiment_config.to_run}_train_plot.png"
            ...
            if not use_kfold_cv and 'X_train' in locals():
